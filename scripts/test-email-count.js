...
async function checkEmailCount() {
  try {
    ...
    if (totalEmails > 0) {
      ...
      // Get recent emails for this account
      const recentAccountEmails = await Email.find({ accountId })
        .sort({ date: -1 })
        .limit(5)
        .select('subject fromEmail date folder')
        .lean();
      
      console.log('📋 Recent emails for account:');
      recentAccountEmails.forEach((email, index) => {
        console.log(`${index + 1}. Subject: ${email.subject}`);
        console.log(`   From: ${email.fromEmail}`);
        console.log(`   Date: ${email.date}`);
        console.log(`   Folder: ${email.folder}`);
